...
locals {
  ...
  worker_managed_roles = {
    ...
    SSMCorePolicy                      = var.ssm_policy_arn
  }
  ...
  services_bucket_prefix = var.aws_profile
  services_bucket_name   = format("%s-%s", local.services_bucket_prefix, local.name)
...
# TODO: remove once we move away from ALBs
...
resource "aws_s3_object" "node_key" {
  acl                    = "bucket-owner-full-control"
  key                    = "${local.name}-node_key.json"
  bucket                 = var.config_storage_bucket
  content                = jsonencode(tls_private_key.node)
...
module "alb-external-sg" {
  source              = "git::git@github.com:terraform-aws-modules/terraform-aws-security-group.git//.?ref=v3.18.0"
  ingress_cidr_blocks = var.office_cidrs
  name                = format("alb-cam-%s", local.name)
  description         = "Security group allows access from the VPN to the ALB"
  vpc_id              = var.vpc_id
  ...
  egress_with_cidr_blocks = [
    {
      ...
      to_port     = -1
      protocol    = -1
      description = "Allows access to the entire VPC"
