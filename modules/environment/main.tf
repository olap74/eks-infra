...
locals {
  ...
  external_subnet_classes_azs = {
    ...
    class => toset(distinct([for subnet_id in data.aws_subnets.external[class].ids : data.aws_subnet.external[subnet_id].availability_zone]))
  }
  ...
  external_subnet_classes_azs_ids = {
    ...
    class => {
      ...
      az => [for id in data.aws_subnets.external[class].ids : id if data.aws_subnet.external[id].availability_zone == az]
    }
  }

  external_subnet_ids = var.external ? toset(flatten([for class in local.external_subnet_classes : data.aws_subnets.external[class].ids])) : []
...
data "aws_subnets" "external" {
  ...
  filter {
    name   = "vpc-id"
    values = [data.aws_vpc.external.0.id]
  }
  tags = {
    class = each.value
  }
